...
class AnalyzeByJSoup(doc: Any) {
    ...
    companion object {
        private val nullSet = emptySet<org.jsoup.nodes.Element?>()
    }
    ...
    data class ElementsSingle(
        ...
        fun getElementsSingle(temp: Element, rule: String): Elements {
            ...
            if (split == '!') { //排除
                ...
                for (pcInt in indexSet) elements[pcInt] = Element("null")

                elements.removeAll(nullSet.map { it ?: return@map it }) //测试过，这样就行
