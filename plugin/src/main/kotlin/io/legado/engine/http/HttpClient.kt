...
object HttpClient {
    ...
    fun get(url: String, headers: Map<String, String> = emptyMap(), useCookie: Boolean = true): HttpResponse {
        return request("GET", url, null, headers, useCookie = useCookie)
    }
    ...
    fun get(
        ...
        return request("GET", url, null, headers, charset, proxy, dnsIp, timeoutMillis, useCookie)
    }

    fun post(url: String, body: String, headers: Map<String, String> = emptyMap(), useCookie: Boolean = true): HttpResponse {
        return request("POST", url, body, headers, useCookie = useCookie)
    }
    ...
    fun post(
        ...
        return request("POST", url, body, headers, charset, proxy, dnsIp, timeoutMillis, useCookie)
    }
    ...
    fun postForm(
        ...
        return request("POST", url, formBody, merged, charset, proxy, dnsIp, timeoutMillis, useCookie)
    }
    ...
    fun postJson(
        ...
        return request("POST", url, jsonBody, merged, charset, proxy, dnsIp, timeoutMillis, useCookie)
    }
    ...
    fun head(
        ...
        return request("HEAD", url, null, headers, null, proxy, dnsIp, timeoutMillis, useCookie)
    }
    ...
    private fun request(
        ...
        val merged = if (useCookie) buildHeaders(url, headers) else {
            val m = headers.toMutableMap()
            if (!m.containsKey("User-Agent") && !m.containsKey("user-agent")) {
                m["User-Agent"] = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
            }
            m
        }
        ...
        val resp = clientFor(proxy, dnsIp, timeoutMillis, useCookie).newCall(builder.build()).execute()
        val respBody = when {
    ...
    private fun clientFor(proxy: String?, dnsIp: String?, timeoutMillis: Long?, useCookie: Boolean = true): OkHttpClient {
        if (proxy.isNullOrBlank() && dnsIp.isNullOrBlank() && timeoutMillis == null) {
            return if (useCookie) client else noCookieClient
        }
        val base = if (useCookie) client else noCookieClient
