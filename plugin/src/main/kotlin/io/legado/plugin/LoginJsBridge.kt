...
class LoginJsBridge(
    ...
    fun connect(urlStr: String): io.legado.engine.http.StrResponse {
        return try {
            io.legado.engine.http.StrResponse(AnalyzeUrl(urlStr, source = bookSource).getStrResponse())
        } catch (e: Exception) {
            io.legado.engine.http.StrResponse(io.legado.engine.http.HttpResponse(urlStr, e.message ?: "", 0))
        }
    }
    ...
    fun connect(urlStr: String, header: String?, callTimeout: Long?): io.legado.engine.http.StrResponse {
        ...
        return try {
            io.legado.engine.http.StrResponse(AnalyzeUrl(urlStr, headerMapF = headerMap, source = bookSource, callTimeout = callTimeout).getStrResponse())
        } catch (e: Exception) {
            io.legado.engine.http.StrResponse(io.legado.engine.http.HttpResponse(urlStr, e.message ?: "", 0))
        }
    }
    ...
    fun startBrowser(url: String, title: String) {
        openBrowserDialog(url, title, null, null, waitForResult = false)
    }
    ...
    fun startBrowserAwait(url: String, title: String, refetchAfterSuccess: Boolean, html: String?): StrResponse {
        if (Looper.myLooper() == Looper.getMainLooper()) {
            openBrowserDialog(url, title, html, null, waitForResult = false)
            return fallbackBrowserResponse(url, html)
        }
        return openBrowserDialog(url, title, html, null, waitForResult = true)
    }
